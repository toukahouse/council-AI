...
export function mountWebUI(app, dirname, accountManager) {
    ...
    app.get('/api/auth/url', async (req, res) => {
        try {
            ...
            // Clear accounts before starting new flow so frontend polling waits correctly
            await saveAccounts(ACCOUNT_CONFIG_PATH, [], {}, 0);
            await accountManager.reload();
            ...
            serverPromise
                .then(async (code) => {
                    try {
                        ...
                        // Empty existing accounts to enforce a single account limit for prototype
                        await saveAccounts(ACCOUNT_CONFIG_PATH, [], {}, 0);
